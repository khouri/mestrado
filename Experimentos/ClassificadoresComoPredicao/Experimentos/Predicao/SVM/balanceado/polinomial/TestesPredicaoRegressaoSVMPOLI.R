...
TestesRegressaoSVRegressionPOLI<-function(ConjuntoTeste, ConjuntoTreino, c, coef, sigma){
  ...
  fit <- ksvm(Class~., 
              ...
              kernel="polydot",
              coef=coef,
              kpar=list(degree=sigma)#, degree= 0.04
              )
  
  # Constroi modelo para classificacao
  predictions <- predict(fit, testing[-position], type="response")
  
  TestaDiferentesLimiares(predictions, testing$Class, "outputSVMpoli",c,coef,sigma)
...
TestaDiferentesLimiares<-function(ValoresPreditos, valoresDeTestes, nomeArquivoSaida, c, coef, sigma){
  ...
  for( x in 1:length(ValoresPreditos)) {
    ...
    print(tb)
    print(paste("c: ", c, "coef ",coef, "degree",sigma))
    print("--------------------------------------------------")
    sink()
...
conjuntoTeste <- leArquivoCSV("/home/toasty/Desktop/Experimentos/Predicao/saidaDesBalanceada_teste.csv") 
conjuntoTreino <- leArquivoCSV("/home/toasty/Desktop/Experimentos/Predicao/saidaDesBalanceada_treino.csv")
...
#    for(coef0 in seq(valoresDeCoef0)){
 ...
 #     print(paste(c = ",valoresDeC[c]" , "gamma=",valoresDeSigma[sigma], "coef0 = ", valoresDeCoef0[coef0]))
      ...
      tryCatch({TestesRegressaoSVRegressionPOLI(conjuntoTeste, conjuntoTreino, 
