...
TestesRegressaoSVRegressionLINEAR<-function(ConjuntoTeste, ConjuntoTreino, c, coef, sigma){
  ...
  #print(tb)
  TestaDiferentesLimiares(predictions, testing$Class, c, coef)
  
}


TestaDiferentesLimiares<-function(ValoresPreditos, valoresDeTestes, c, coef){
  ...
  for( x in 1:length(ValoresPreditos)) {
    ...
    #print(matrizTmp)
    #Grava arquivo
    sink(paste('/home/toasty/Desktop/RBooks/Experimentos/Predicao/outputSVMLinear.txt'), append=TRUE)
    print(tb)
    print(paste("c: ", c, " coef: ", coef))
    print("--------------------------------------------------")
    sink()
...
for(c in seq(valoresDeC)){
  for(sigma in seq(valoresDeSigma)){
    for(coef0 in seq(valoresDeCoef0)){
      ...
      tryCatch({TestesRegressaoSVRegressionLINEAR(conjuntoTeste, conjuntoTreino, 
