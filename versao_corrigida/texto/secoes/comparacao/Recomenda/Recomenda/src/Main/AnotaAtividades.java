...
public class AnotaAtividades {

	private ArrayList<String> listaDeAtividades = new ArrayList<String>();

	private void inicializaArrayString(String [] wfs){
	...
	public AnotaAtividades(){
		this.preencheAtividades();
	}

	public void anotadorDeAtividades(String[][] matrizCSV) {

		String [] headers = new String[281];
		String [] ontologias = new String[73];
		String [] wfs = new String[73];
		inicializaArrayString(wfs);

		for (int i = 0; i < matrizCSV[0].length; i++) {//Pega headers
			headers[i] = matrizCSV[0][i];
		}
		
		for (int i = 0; i < matrizCSV.length; i++) {//Pega Ontologias
			ontologias[i] = matrizCSV[i][280];
		}

		for (int i = 0; i < matrizCSV[0].length-2; i++) {//Pega Colunas
			for (int j = 1; j < (matrizCSV.length); j++) {//Pega Linhas uso j+1 para acessar a ontologia do trem
				
				//System.out.print(matrizCSV[j][i] + ", ");
				if(matrizCSV[j][i].equals("1")){

					wfs[j] = wfs[j] + ", "+ ontologias[j];
					headers[i] = headers[i] + ", " + ontologias[j];
				}
			}
		//	System.out.println();
		}
		
		imprimeArrayCSV(headers);
	}
